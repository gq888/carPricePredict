...
class ComprehensiveFeatureEngineering:
    ...
    def _apply_dimensionality_reduction_transforms(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 应用特征聚类（如果在训练时创建了）
        if 'feature_kmeans' in self.fitted_transformers and 'feature_clusters' in self.fitted_transformers:
            cluster_labels = self.fitted_transformers['feature_clusters']
            ...
            if len(numeric_cols) > 0 and len(numeric_cols) == len(cluster_labels):
                # 重新创建聚类特征（使用保存的聚类标签）
                for cluster_id in range(n_clusters):
                    cluster_features = [numeric_cols[i] for i in range(len(numeric_cols)) 
                                      if cluster_labels[i] == cluster_id]
                    if len(cluster_features) > 0:
                        # 使用聚类内特征的平均值和标准差作为代表性特征
                        numeric_cluster_features = []
                        for feature in cluster_features:
                            if feature in df.columns and pd.api.types.is_numeric_dtype(df[feature]):
                                numeric_cluster_features.append(feature)
                        ...
                        if len(numeric_cluster_features) > 0:
                            df_reduced[f'cluster_{cluster_id}_mean'] = df[numeric_cluster_features].mean(axis=1)
                            df_reduced[f'cluster_{cluster_id}_std'] = df[numeric_cluster_features].std(axis=1)
            elif len(numeric_cols) != len(cluster_labels):
                print(f"  警告: 数值特征数量不匹配 (当前: {len(numeric_cols)}, 训练时: {len(cluster_labels)})，跳过聚类特征")
